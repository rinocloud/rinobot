...
/***/ function(module, exports, __webpack_require__) {
	...
	var _electron = __webpack_require__(300);
	
	var _menu = __webpack_require__(301);
	
	var _menu2 = _interopRequireDefault(_menu);
	
	var _rpc = __webpack_require__(302);
	
	var _rpc2 = _interopRequireDefault(_rpc);
	
	var _bot = __webpack_require__(306);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var genUid = __webpack_require__(304);
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = function (rpc) {
	  var child = _child_process2.default.fork('./fork');
	
	  child.send('heartbeat');
	  rpc.on('heartbeat', function () {
	    console.log('heartbeat');
	  });
	
	  rpc.on('watch', function (args) {
	    child.send('watch', args);
	  });
	  rpc.on('unwatch', function (args) {
	    child.send('unwatch', args);
	  });
	
	  child.on('watcher ready', function (args) {
	    rpc.emit('watcher ready', args);
	  });
	
	  child.on('watcher started', function (args) {
	    rpc.emit('watcher started', args);
	  });
	
	  child.on('watcher set total files', function (args) {
	    rpc.emit('watcher set total files', args);
	  });
	
	  child.on('watcher set processed files', function (args) {
	    rpc.emit('watcher set processed files', args);
	  });
	
	  child.on('pipeline started', function (args) {
	    rpc.emit('pipeline started', args);
	  });
	
	  child.on('pipeline complete', function (args) {
	    rpc.emit('pipeline complete', args);
	  });
	
	  child.on('pipeline error', function (args) {
	    rpc.emit('pipeline error', args);
	  });
	
	  child.on('pipeline log', function (args) {
	...
	var _child_process = __webpack_require__(307);
