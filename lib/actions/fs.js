...
export const clickSetBasePath = (dirPath) => (dispatch) => {
  mixpanel.track('Choose folder (base path)', {
    Path: dirPath
  })
  dispatch(setBasePath(dirPath))
}
...
export const setCurrentPath = (dirPath) => (dispatch) => {
  // mixpanel.track('Choose folder (current path)', {
  //   Path: dirPath
  // })
...
export const refreshDir = (cb) => (dispatch, getState) => {
  dispatch(readDir(getState().fs.currentPath, cb))
}


export const readDir = (dirPath, cb) => (dispatch) => {
    ...
    async.map(
      ...
      (er, stats) => {
        const dir = _.reduce(items, (collector, fname, index) => {
          ...
          if (pt.extname(fname) === '.yaml') return collector
...
export const processSelected = () => (dispatch, getState) => {
  ...
  if (_.find(pipelineForm.pipelines[0].tasks, (o) => { return o.name === null })) {
    return dispatch(uiActions.addError('All tasks need to be defined.'))
  }

  console.log('running task')

  mixpanel.track('Run Task', {
    Files: files,
    'Task Names': _.map(pipelineForm.pipelines[0].tasks, 'name'),
  })
...
export const newSnippet = (name) => (dispatch, getState) => {
  ...
  fs.writeFile(fname, snippetTemplate(name), (err) => {
    ...
    mixpanel.track('Python snippet created', { name })
