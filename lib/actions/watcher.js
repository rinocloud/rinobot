...
/*
  Respond to RPC to set pipeline
*/

export const pipelineStarted = ({ index, pipePath }) => (dispatch, getState) => { // eslint-disable-line
  dispatch(addLogs({ index, logs: [`${pipePath} started`] }))
  dispatch(setBusy(index))
}

export const pipelineComplete = ({ index, pipePath }) => (dispatch, getState) => { // eslint-disable-line
  dispatch(addLogs({ index, logs: [`${pipePath} complete`] }))
  dispatch(unsetBusy(index))
}

export const taskStarted = ({ index, pipePath, task }) => (dispatch, getState) => { // eslint-disable-line
  dispatch(addLogs({ index, logs: [`${pipePath} task started`] }))
  dispatch(setBusy(index))
}
...
export const taskComplete = ({ index, pipePath, command, args, match }) => (dispatch, getState) => { // eslint-disable-line
  ...
  dispatch(addLogs({ index, logs: [`${pipePath}, task ${command} complete`] }))
  dispatch(unsetBusy(index))
}
